...
def create_app(test_config=None):
    ...
    def delete_question(question_id):
        ...
        """
        try:
            this_question = Question.query.filter_by(
    ...
    def search_questions():
        ...
        try:
            ...
            found_questions = Question.query.filter(
                Question.question.ilike(q_string)).all()
            return jsonify({
                'success': True,
    ...
    def get_all_questions_per_category(category_id):
        ...
        """
        try:
            all_questions_for_cat = Question.query.order_by(
                Question.id).filter(Question.category == category_id).all()
            if not all_questions_for_cat:
                abort(404)
            else:
                return jsonify({
                    'success': True,
                    'questions': [c.format() for c in all_questions_for_cat]
                })
        except Exception:
            abort(422)
    ...
    def play_game(category_id):
        """Endpoint to receive questions for a category_id without
            blacklist (POST)
        ARGS:
            category_id - int
            JSON (previous_question) - list of backlist questions
                which we don´t wanted to return
        RETURNS:
        ...
        try:
            body = request.get_json()
            prev_quest = body.get('previous_question', None)
            except_q = Question.query.filter(
                Question.category == category_id, ~Question.id.in_(prev_quest))
            return jsonify({
                'success': True,
                'list_of_questions_to_play': [i.format() for i in except_q]
            })
        except Exception:
            abort(422)
